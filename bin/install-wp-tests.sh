...
install_depencency() {
	...
	#Gravity PDF
	if [ ! -f "$TMPDIR/gravity-forms-pdf-extended.zip" ]; then
	  download https://downloads.wordpress.org/plugin/gravity-forms-pdf-extended.latest-stable.zip "$TMPDIR/gravity-forms-pdf-extended.zip"
	  unzip -q -o "$TMPDIR/gravity-forms-pdf-extended.zip" -d "$TMPDIR"
	fi
}
